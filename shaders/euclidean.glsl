...
vec3 phongModel(vec4 samplePoint, vec4 T, vec4 N, mat4 totalFixMatrix, mat4 invObjectBoost, bool isGlobal){
    ...
    if(isGlobal)
      return baseColor = texcube(texture, samplePoint, N, 4.0, cellBoost * invObjectBoost).xyz; //Helps see what's wrong with texcube currently - remove return when fixed
    else
