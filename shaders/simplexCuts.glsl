...
float localSceneSDF(vec4 samplePoint){
    ...
    if(cut4 == 1){
        ...
        vertexSphere = sphereSDF(abs(samplePoint), vertexPos, vertexSurfaceOffset);
    }
    else if(cut4 == 2) {
        vertexSphere = horosphereHSDF(abs(samplePoint), idealCubeCornerKlein, vertexSurfaceOffset);
    }
        ...
        vertexSphere = geodesicPlaneHSDF(abs(samplePoint), dualPoint, vertexSurfaceOffset);
    }
